...
function mostrarResumenPedido() {
    ...
    if (itemsResumen && carrito.length > 0) {
        ...
        if (totalPedido) totalPedido.textContent = `${total.toFixed(2)}€`;
    }
}
...
// Agregar la nueva funcionalidad del formulario de contacto
document.addEventListener('DOMContentLoaded', () => {
    // Mantener la funcionalidad existente de productos
    mostrarProductos();
    
    // Agregar el nuevo manejador del formulario de contacto
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Función para mostrar productos (mantener la existente)
    function mostrarProductos() {
            ...
            contenedorProductos.innerHTML = productos.map(producto => `
                <div class="producto">
                    ...
                    <div class="producto-info">
                        ...
                        <button class="boton-principal agregar-carrito" data-id="${producto.id}">
    ...
    // Llamar a mostrarProductos si estamos en la página de productos
    if (document.querySelector('.grid-productos')) {
        mostrarProductos();
    }

    // Navegación del formulario de pago
    const formularioPago = document.getElementById('formulario-pago');
    if (formularioPago) {
        const secciones = formularioPago.querySelectorAll('.seccion-formulario');
        const pasos = formularioPago.querySelectorAll('.paso');
        const botonesAnterior = formularioPago.querySelectorAll('.anterior');
        const botonesSiguiente = formularioPago.querySelectorAll('.siguiente');
        ...
        const validarPasoActual = (paso) => {
            ...
            const camposRequeridos = seccionActual.querySelectorAll('input[required]');
            let valido = true;
